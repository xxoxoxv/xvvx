...
class RoyalDecree:
    ...
    def verify(self, crown: Crown | None = None) -> None:
        """تحقق من **أصالة** المرسوم وحدها: توقيع Ed25519 مقابل مفتاح التاج.

        هذه الدالة تجيب سؤالًا واحدًا: **هل هذا هو الملك؟** ولا تجيب أبدًا سؤال
        «هل يُسمح للملك؟» — لأن السؤال الثاني سلطة فوق الملك، وهي ممنوعة.

        تصحيح E2.1: كان هنا فحصٌ يرفض المرسوم إن مسّ بندًا محصَّنًا، **قبل**
        النظر في التوقيع. وذلك كان نقضًا مستورًا داخل التحقق من الأصالة: توقيع
        صحيح من التاج نفسه كان يُرفض بحجة أنه غير أصيل. وقد فُصل الفحصان: الأصالة
        هنا، وأثر المرسوم على السيادة صار حدثًا أمنيًّا يُسجَّل ولا يمنع
        (`sovereignty_alterations`). فالتوثيق ليس تفويضًا من سلطة أعلى، والتحصين
        لم يكن ليحمي الملك من مرسوم منتزَع إكراهًا — لأن مَن يملك المفتاح يملك
        صياغة المرسوم بأي هدف آخر. الحماية الحقيقية موضعها المفتاح والسجل
        والخلافة، لا نقضٌ يعلو على التاج.
        """
    ...
    def sovereignty_alterations(self) -> tuple[str, ...]:
        """البنود المُنشئة للسيادة التي يمسّها هذا المرسوم — للتسجيل لا للمنع.

        مرسوم يمسّ سيادة التاج نفسها حدث جسيم يستحق أعلى درجات الإشهار: يُسجَّل
        بحدث أمني `SOVEREIGNTY_ALTERING_DECREE` قبل التنفيذ. ولا يُمنع — لأن
        منعه يعني أن هذه الوحدة سلطة فوق الملك.
        """
        return immune_clauses_touched(self.all_targets())
