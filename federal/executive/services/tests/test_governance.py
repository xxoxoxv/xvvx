...
AUTH_HEADERS = {
    "Authorization": "Bearer " + create_access_token("tester", ["governance:read", "governance:write"])
}
...
def test_check_promotion_policy_allows_valid() -> None:
    ...
    resp = client.post("/v1/policies/check", headers=AUTH_HEADERS, json={
...
def test_activate_kill_switch() -> None:
    """تفعيل مفتاح الإيقاف."""
    resp = client.post("/v1/system/kill-switch", headers=AUTH_HEADERS, json={
        "level": "halt", "reason": "اختبار طارئ", "activated_by": "admin"
    })
    assert resp.status_code == 200
...
def test_reset_kill_switch() -> None:
    ...
    client.post("/v1/system/kill-switch", headers=AUTH_HEADERS, json={
        "level": "alert", "reason": "test", "activated_by": "admin"
    })
...
def test_kill_switch_rejects_invalid_level() -> None:
    ...
    resp = client.post("/v1/system/kill-switch", headers=AUTH_HEADERS, json={
        "level": "invalid", "reason": "test", "activated_by": "admin"
    })
...
def test_check_gate_passed() -> None:
    ...
    resp = client.post(f"/v1/promotions/{promo_id}/gates", headers=AUTH_HEADERS,
...
def test_all_gates_passed_promotes() -> None:
    ...
    for gate in ["evaluation", "shadow", "canary", "human_approval", "activation"]:
        client.post(f"/v1/promotions/{promo_id}/gates", headers=AUTH_HEADERS,
...
def test_create_canary() -> None:
    ...
    resp = client.post("/v1/canary", headers=AUTH_HEADERS,
...
def test_get_canary() -> None:
    """استرجاع Canary."""
    create = client.post("/v1/canary", headers=AUTH_HEADERS,
                          json={"model_id": "model-c", "traffic_percentage": 10})
...
def test_update_canary_metrics_healthy() -> None:
    ...
    canary_id = create.json()["canary_id"]
    resp = client.patch(f"/v1/canary/{canary_id}/metrics", headers=AUTH_HEADERS,
...
def test_canary_auto_rollback_high_errors() -> None:
    ...
    create = client.post("/v1/canary", headers=AUTH_HEADERS,
                          json={"model_id": "model-bad", "traffic_percentage": 5})
    canary_id = create.json()["canary_id"]
    resp = client.patch(f"/v1/canary/{canary_id}/metrics", headers=AUTH_HEADERS,
...
def test_list_canaries() -> None:
    ...
    client.post("/v1/canary", headers=AUTH_HEADERS,
                json={"model_id": "model-list", "traffic_percentage": 5})
...
def test_audit_log_has_entries() -> None:
    ...
    client.post("/v1/policies/check", headers=AUTH_HEADERS, json={
        "policy_name": "budget_policy", "context": {"daily_spend_usd": 50}
    })
