...
def test_update_agent_state_pause() -> None:
    ...
    agents = client.get("/v1/agents", headers=AUTH_HEADERS).json()
    agent_id = agents[0]["agent_id"]
    resp = client.post(f"/v1/agents/{agent_id}/state", headers=AUTH_HEADERS,
                      json={"state": "paused"})
    assert resp.status_code == 200
...
def test_update_agent_state_activate() -> None:
    ...
    agents = client.get("/v1/agents", headers=AUTH_HEADERS).json()
    agent_id = agents[0]["agent_id"]
    client.post(f"/v1/agents/{agent_id}/state", headers=AUTH_HEADERS,
                json={"state": "paused"})
    resp = client.post(f"/v1/agents/{agent_id}/state", headers=AUTH_HEADERS,
                      json={"state": "active"})
    assert resp.status_code == 200
...
def test_kill_switch_activate() -> None:
    ...
    resp = client.post("/v1/kill-switch", headers=AUTH_HEADERS,
...
def test_kill_switch_reset() -> None:
    ...
    client.post("/v1/kill-switch", headers=AUTH_HEADERS,
                json={"level": "alert", "reason": "اختبار", "activated_by": "tester"})
...
def test_approval_sign() -> None:
    ...
    resp = client.post("/v1/approval", headers=AUTH_HEADERS,
...
def test_approval_reject() -> None:
    """زر الرفض يعمل."""
    resp = client.post("/v1/approval", headers=AUTH_HEADERS,
                       json={"decision": "reject", "signed_by": "tester"})
    assert resp.status_code == 200
...
def test_approval_publishes_event() -> None:
    ...
    client.post("/v1/approval", headers=AUTH_HEADERS,
                 json={"decision": "approve", "signed_by": "tester"})
