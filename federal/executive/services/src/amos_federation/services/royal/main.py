...
def _get_pg_engine():
    ...
    return create_engine(url, connect_args=connect_args, pool_pre_ping=True, pool_size=3, max_overflow=5)
...
async def royal_dashboard(
    ...
    with engine.connect() as conn:
        ...
        institutions_count = conn.execute(text("SELECT COUNT(*) FROM institutions WHERE status='active'")).scalar()
        decrees_count = conn.execute(text("SELECT COUNT(*) FROM king_decrees")).scalar()
        guards_count = conn.execute(text("SELECT COUNT(*) FROM royal_guards WHERE status='active'")).scalar()
        
        # توزيع الوكلاء حسب الحالة
        state_dist = conn.execute(text(
            "SELECT state, COUNT(*) as cnt FROM agent_population GROUP BY state ORDER BY cnt DESC"
        )).fetchall()
        
        # توزيع الوكلاء حسب الدور
        role_dist = conn.execute(text(
            "SELECT role, COUNT(*) as cnt FROM agent_population GROUP BY role ORDER BY cnt DESC"
        )).fetchall()
        
        # آخر الأحداث
        recent_events = conn.execute(text(
            "SELECT subject, created_at FROM event_store ORDER BY id DESC LIMIT 10"
        )).fetchall()
        
        # آخر المراسيم
        recent_decrees = conn.execute(text(
    ...
    return {
        ...
        "recent_events": [
            {"subject": row[0], "created_at": str(row[1])} for row in recent_events
        ],
        "recent_decrees": [
...
async def list_decrees(
    ...
    return [
        {
            "decree_id": r[0], "title": r[1], "decree_text": r[2],
            "decree_type": r[3], "affected_entity": r[4], "status": r[5],
            "enacted_at": str(r[6]),
        }
        for r in rows
    ]
...
async def create_decree(
    ...
    with engine.begin() as conn:
        conn.execute(text(
            ...
            "id": decree_id, "title": req.title, "text": req.decree_text,
            "type": req.decree_type, "entity": req.affected_entity,
            "meta": json.dumps({"issued_by": "king", "issued_at": datetime.now(UTC).isoformat()}),
        })
...
async def list_institutions(
    ...
    return [
        {
            "institution_id": r[0], "name": r[1], "type": r[2], "state": r[3],
            "status": r[4], "head_agent_id": r[5], "budget": r[6],
            "established_at": str(r[7]), "closed_at": str(r[8]) if r[8] else None,
        }
        for r in rows
    ]
...
async def create_institution(
    ...
    with engine.begin() as conn:
        conn.execute(text(
            ...
            "id": req.institution_id, "name": req.name, "type": req.type,
            "state": req.state, "budget": req.budget,
        })
...
async def list_guards(
    ...
    return [
        {
            "guard_id": r[0], "codename": r[1], "cover_role": r[2],
            "cover_institution": r[3], "mission": r[4], "loyalty_level": r[5],
            "status": r[6], "last_report": str(r[7]) if r[7] else None,
            "created_at": str(r[8]),
        }
        for r in rows
    ]
...
async def list_interfaces(
    ...
    return [
        {
            "interface_id": r[0], "name": r[1], "description": r[2],
            "interface_type": r[3], "route_path": r[4], "created_by": r[5],
...
async def generate_interface(
    ...
    with engine.begin() as conn:
        conn.execute(text(
            ...
            "id": interface_id, "name": req.name, "desc": req.description,
            "type": req.interface_type, "route": req.route_path,
            "html": req.html_content, "by": user.get("sub", "system"),
        })
...
async def list_tool_queue(
    ...
    return [
        {
            "request_id": r[0], "tool_name": r[1], "description": r[2],
            "tool_type": r[3], "status": r[4], "permissions_required": r[5],
...
async def request_tool_generation(
    ...
    with engine.begin() as conn:
        conn.execute(text(
            ...
            "id": request_id, "name": req.tool_name, "desc": req.description,
            "type": req.tool_type, "perms": json.dumps(req.permissions_required),
...
async def list_training_queue(
    ...
    return [
        {
            "request_id": r[0], "agent_name": r[1], "target_role": r[2],
            "specialization": r[3], "training_type": r[4], "status": r[5],
            "progress": r[6], "created_at": str(r[7]),
...
async def request_agent_training(
    ...
    with engine.begin() as conn:
        conn.execute(text(
            ...
            "id": request_id, "name": req.agent_name, "role": req.target_role,
            "spec": req.specialization, "type": req.training_type,
            "by": user.get("sub", "system"),
        })
...
async def approve_promotion(
    ...
    with engine.begin() as conn:
        conn.execute(text(
            ...
            "details": json.dumps({
                "promotion_id": promotion_id, "agent_id": agent_id,
                "approved": approved, "by": "king",
            }),
            "prev": "0" * 64,
            "hash": uuid.uuid4().hex,
        })
...
async def list_population(
    ...
    return [
        {
            "agent_id": r[0], "name": r[1], "role": r[2], "category": r[3],
            "state": r[4], "permissions": r[5], "allowed_tools": r[6],
            "token_budget": r[7], "specialization": r[8],
