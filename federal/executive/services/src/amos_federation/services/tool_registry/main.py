...
الهدف: توفير هيكل تشغيل واضح لخدمة tool-registry
النطاق: خدمة tool-registry ضمن Sprint الحالي
...
async def not_implemented() -> None:
    ...
    raise HTTPException(
        ...
        detail="وظيفة حل الأدوات مؤجلة إلى الأسبوع 7-8: Agent Runtime + Tools",
    )


_service = SERVICES["tool-registry"]
app = create_service_app(_service["name"], _service["port"], "حل الأدوات", [router])
