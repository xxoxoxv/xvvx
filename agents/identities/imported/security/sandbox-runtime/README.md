# Sandbox Runtime (Anthropic) — مواطن مستورد (sandbox-runtime)

## التعريف
`Sandbox Runtime (Anthropic)` مورد خارجي مستورد إلى الفدرالية كـ**موظف مرشَّح** لا كعضو عامل. سُجِّل
تحت المعرّف `imported-security-sandbox-runtime` من مصدره [https://github.com/anthropics/sandbox-runtime](https://github.com/anthropics/sandbox-runtime)، وصُنِّف أصلًا في
«أمان وحوكمة»، وأُسكِن في مجال `security`،
وتخصصه الدقيق **sandbox-runtime**، وخُصِّص له المكان
`security` داخل بنية الدولة. لم يُنسخ كود مصدره
