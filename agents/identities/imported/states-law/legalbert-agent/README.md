# LegalBERT-Agent — مواطن مستورد (legalbert-agent)

## التعريف
`LegalBERT-Agent` مورد خارجي مستورد إلى الفدرالية كـ**موظف مرشَّح** لا كعضو عامل. سُجِّل
تحت المعرّف `imported-law-legalbert-agent` من مصدره [https://github.com/legalbert/legalbert-agent](https://github.com/legalbert/legalbert-agent)، وصُنِّف أصلًا في
«وكلاء متخصصون (قانون)»، وأُسكِن في مجال `states-law`،
وتخصصه الدقيق **legal**، وخُصِّص له المكان
`states/law` داخل بنية الدولة. لم يُنسخ كود مصدره
