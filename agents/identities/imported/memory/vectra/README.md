# Vectra — مواطن مستورد (vectra)

## التعريف
`Vectra` مورد خارجي مستورد إلى الفدرالية كـ**موظف مرشَّح** لا كعضو عامل. سُجِّل
تحت المعرّف `imported-memory-vectra` من مصدره [https://github.com/StevenWeathers/vectra](https://github.com/StevenWeathers/vectra)، وصُنِّف أصلًا في
«الذاكرة والبيانات»، وأُسكِن في مجال `memory`،
وتخصصه الدقيق **vector-search**، وخُصِّص له المكان
`memory` داخل بنية الدولة. لم يُنسخ كود مصدره
