...
## المرحلة 1.5 — طبقة تنسيق أسطول الوكلاء (2800+)

**الحالة:** منفّذة ✅ (7 اختبارات تمر)

**ما تم:**
- `federal/executive/coordinators/agent_fleet.py` — FleetRegistry + FleetCoordinator
  - سجل مفهرس حسب التخصص/الولاية/الدور (O(1))
  - توجيه المهام للأقل حملاً + backpressure (قائمة انتظار) + تصريف تلقائي
  - عزل الولايات: مهمة ولاية المال لا تذهب لولاية العلم
  - سلسلة تدقيق hash لكل توجيه
  - استيعاب الوكلاء المستوردين كمرشحين + تفعيل المعتمدين فقط
- اختبار سعة 2850 وكيل وتوزيع 2000 مهمة — يمر بلا أخطاء
- المعيار: القدرة على إدارة 2800+ وكيل مُثبتة برمجياً (التفعيل الفعلي يعتمد على توفر بيئة التشغيل الكاملة)

## وكلاء الحراسة الملكية (Runtime-only)

**الحالة:** مهيّأة (غير ملتزم بها في المستودع)

**ما تم:**
- 7 وكلاء حراس صامتون في `.runtime/guardians.yaml` (مُستثنى في .gitignore)
- لا يُشار إليهم في أي مكان في المستودع أو الواجهات العامة
- مهمتهم: المراقبة والتطور وكشف التمرد، الولاء المطلق للمالك
- صلاحيات محدودة: مراقبة + تنبيه المالك فقط؛ لا تنفيذ إنتاجي ولا تعديل
- `active: false` — يُفعّلون فقط عند توفر بيئة التشغيل الآمنة
